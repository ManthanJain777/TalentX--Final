  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        console.error('Error fetching employer projects for messages:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/messages', {
        projectId,
        senderId: user?.id,
        receiverId: activeProject.freelancerId || 'CANDIDATE',
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-xs text-ink-soft">Direct milestone messaging with matched candidates</p>
        </div>
        <EmptyState 
          title="No Active Message Channels" 
          description="Messages will appear here once you have an active governed milestone project." 
        />
      </div>
    );
  }
    ...
    <div className="h-[calc(100vh-140px)] min-h-[550px] glass-card p-0 overflow-hidden bg-white/95 border-cover/15 shadow-xl flex flex-col md:flex-row animate-in fade-in duration-300">
      ...
      <div className="w-full md:w-80 border-r border-cover/10 flex flex-col bg-page/40">
        <div className="p-4 border-b border-cover/10">
          ...
          <p className="text-xs text-ink-soft">Encrypted direct channels</p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="text-[11px] font-mono text-gold-dark font-semibold mb-1">
                  Talent ID: {proj.freelancerId?.slice(-6) || 'Candidate'}
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-ink-soft font-mono">
                Candidate ID: <strong className="text-gold-dark">{activeProject?.freelancerId}</strong>
              </div>
            </div>
          </div>
