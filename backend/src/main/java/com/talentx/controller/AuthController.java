...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<JwtAuthenticationResponse>> login(@Valid @RequestBody LoginRequest request, HttpServletRequest httpRequest) {
        ...
        if (ipAddress == null || ipAddress.isBlank() || "unknown".equalsIgnoreCase(ipAddress)) ipAddress = httpRequest.getRemoteAddr();
        authService.captureLoginDetails(request.getEmail(), ipAddress, httpRequest.getHeader("User-Agent"));

        Object userObj = authService.getMe(userPrincipal.getUsername()).get("user");
        JwtAuthenticationResponse response = new JwtAuthenticationResponse(null, userPrincipal.getUserId(), role, userObj);

        ResponseCookie cookie = ResponseCookie.from("talentx_token", jwt)
                .httpOnly(true).secure(secureCookie).path("/").maxAge(24 * 60 * 60).sameSite(secureCookie ? "None" : "Lax").build();

        SecurityContextHolder.getContext().setAuthentication(authentication);
        return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString()).body(ApiResponse.success("Login successful", response));
    }

    @PostMapping("/logout")
    public ResponseEntity<ApiResponse<String>> logout() {
        ResponseCookie cookie = ResponseCookie.from("talentx_token", "")
                .httpOnly(true).secure(secureCookie).path("/").maxAge(0).sameSite(secureCookie ? "None" : "Lax").build();
        return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString()).body(ApiResponse.success("Logout successful", null));
    }

    @PostMapping("/register/candidate")
    public ResponseEntity<ApiResponse<String>> registerCandidate(@Valid @RequestBody RegisterCandidateRequest request) {
        authService.registerCandidate(request.getEmail(), request.getPassword(), request.getFirstName(), request.getLastName());
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success("Candidate registered successfully. Welcome email sent.", null));
    }

    @PostMapping("/register/employer")
    public ResponseEntity<ApiResponse<String>> registerEmployer(@Valid @RequestBody RegisterEmployerRequest request) {
        authService.registerEmployer(request.getEmail(), request.getPassword(), request.getCompanyName());
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success("Employer registered successfully. Welcome email sent.", null));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getMe(Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated() || !(authentication.getPrincipal() instanceof com.talentx.security.UserPrincipal)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error("Unauthorized: No active session"));
        }
        var principal = (com.talentx.security.UserPrincipal) authentication.getPrincipal();
        return ResponseEntity.ok(ApiResponse.success("User fetched successfully", authService.getMe(principal.getUsername())));
    }
}
